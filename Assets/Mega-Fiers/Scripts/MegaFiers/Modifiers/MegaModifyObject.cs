...
public class MegaModifyObject : MegaModifiers
{
	...
	void Start()
	{
		if ( cachedMesh == null )
            try {
			    cachedMesh = (Mesh)Mesh.Instantiate(FindMesh(gameObject, out sourceObj));
            } catch (NullReferenceException nfe) { // by Simon
                //Debug.LogError ("I caught you NPE: " + nfe.Message);
            } catch (ArgumentException cne) { // by Simon
                // We got a NPE here but I never figured out why and it didn't seem to cause any issues.
                // It appeared sometime after I added the logic to destroy meshes to prevent memory leaks.
                //Debug.LogError ("I caught you NPE: " + cne.Message);
            }
