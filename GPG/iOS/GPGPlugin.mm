...
extern "C" void GPG_SaveToCloud(int keyNum, const char *bytes, int len)
{
    ...
    [model updateForKey:dataKey completionHandler:^(GPGAppStateWriteStatus status, NSError *error) {
        ...
        if (keyNum == 0) { // current balance
            // simon hack: return state with higher balance
            NSString* localBalanceStr = [[NSString alloc] initWithData:localState encoding:NSUTF8StringEncoding];
            NSString* cloudBalanceStr = [[NSString alloc] initWithData:remoteState encoding:NSUTF8StringEncoding];
            NSLog(@"GPG_SaveToCloud CONFLICT! local balance: %@ <-> cloud balance: %@", localBalanceStr, cloudBalanceStr);
            int localBalanceInt = [localBalanceStr intValue];
            int cloudBalanceInt = [cloudBalanceStr intValue];
            if (localBalanceInt >= cloudBalanceInt) {
                NSLog(@"Resolving with local balance: %d", localBalanceInt);
                return localState;
            } else {
                NSLog(@"Resolving with cloud balance: %d", cloudBalanceInt);
                return remoteState;
            }
            
            //return localState;
        } else if (keyNum == 1) { // ads or no ads (pro / not pro)
            NSString* localAdsProStr = [[NSString alloc] initWithData:localState encoding:NSUTF8StringEncoding];
            NSString* cloudAdsProStr = [[NSString alloc] initWithData:remoteState encoding:NSUTF8StringEncoding];
            NSLog(@"GPG_SaveToCloud CONFLICT! local ads/pro: %@ <-> cloud ads/pro: %@", localAdsProStr, cloudAdsProStr);
            if ([localAdsProStr isEqualToString:@"pro"]) {
                NSLog(@"Resolving as localAdsProStr == pro");
                return localState;
            } else if ([cloudAdsProStr isEqualToString:@"pro"]) {
                NSLog(@"Resolving as cloudAdsProStr == pro");
                return remoteState;
            } else {
                NSLog(@"Resolving as localAdsPro == %@ (btw cloud == %@)", localAdsProStr, cloudAdsProStr);
                return localState;
            }
        } else {
            return localState;
...
extern "C" void GPG_LoadFromCloud(int keyNum, const char *bytes, int len)
{
    ...
    [model loadForKey:dataKey completionHandler:^(GPGAppStateLoadStatus status, NSError *error) {
        ...
        if (keyNum == 0) { // current balance
            ...
            NSString* localBalanceStr = [[NSString alloc] initWithData:localState encoding:NSUTF8StringEncoding];
            NSString* cloudBalanceStr = [[NSString alloc] initWithData:remoteState encoding:NSUTF8StringEncoding];
            NSLog(@"GPG_LoadFromCloud CONFLICT! local balance: %@ <-> cloud balance: %@", localBalanceStr, cloudBalanceStr);
            int localBalanceInt = [localBalanceStr intValue];
            int cloudBalanceInt = [cloudBalanceStr intValue];
            if (localBalanceInt >= cloudBalanceInt) {
                NSLog(@"Resolving with local balance: %d", localBalanceInt);
            ...
            NSString* localAdsProStr = [[NSString alloc] initWithData:localState encoding:NSUTF8StringEncoding];
            NSString* cloudAdsProStr = [[NSString alloc] initWithData:remoteState encoding:NSUTF8StringEncoding];
            NSLog(@"GPG_LoadFromCloud CONFLICT! local ads/pro: %@ <-> cloud ads/pro: %@", localAdsProStr, cloudAdsProStr);
            if ([localAdsProStr isEqualToString:@"pro"]) {
                NSLog(@"Resolving as localAdsProStr == pro");
                return localState;
            } else if ([cloudAdsProStr isEqualToString:@"pro"]) {
