...
public class NerdGPG : MonoBehaviour {
    ...
    public const string appID = "635071630698";
	...
	private byte[] key0CloudData = new byte[50]; // 128k is max length. set it to what you think is you need
	private byte[] key1CloudData = new byte[10]; // 128k is max length. set it to what you think is you need
	private byte[] key2CloudData = new byte[0]; // 128k is max length. set it to what you think is you need
	private byte[] key3CloudData = new byte[0]; // 128k is max length. set it to what you think is you need
#endif
...
#endif // UNITY_UNITY_ANDROID && !UNITY_EDITOR && !UNITY_EDITOR		
	}
...
#region GPGCallbacks
    ...
    {
        Debug.Log ("Received GPGAuthResult: " + result);
        // success/failed
        if (result == "success") {
        ...
        if (!PlayerPrefs.HasKey("gpg")) {
            Debug.Log ("Google Play Games signed on for first time. Bonus of " + "+$" + Dealer.GPG_LOGON_BONUS.ToString("n0"));
            ...
            FlurryAnalytics.Instance().LogEvent (Consts.FE_GPG_LOGON);
        }
    }
    ...
    public void OnGPGCloudLoadResult(string result)
    {
        ...
        // Simon hacks:
        if ("0".Equals(resArr[1])) {
            Debug.Log ("Loaded balance (key 0) from cloud...");
            int cloudBalance = int.Parse(System.Text.Encoding.UTF8.GetString(getKeyLoadedData(0)));
            ...
            FlurryAnalytics.Instance().LogEvent(Consts.FE_GPG_RESTORE_CLOUD_BALANCE);
            GameState.Instance.updateGUIText();
        } else if ("1".Equals (resArr[1])) {
            Debug.Log ("Loaded ads/pro (key 1) from cloud...");
            string adsProStr = System.Text.Encoding.UTF8.GetString(getKeyLoadedData(0));
            if ("pro".Equals(adsProStr)) {
                ...
                FlurryAnalytics.Instance().LogEvent(Consts.FE_GPG_RESTORE_CLOUD_PRO_VERSION);
            }
        }
    }
    ...
    public void OnGPGSubmitScoreResult(string result)
    {
        ...
        // Simon hacks:
        FlurryAnalytics.Instance().LogEvent (Consts.FE_GPG_SCORE_POST);
    }
